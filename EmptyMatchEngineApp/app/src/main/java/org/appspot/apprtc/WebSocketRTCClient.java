...
public class WebSocketRTCClient implements AppRTCClient, WebSocketChannelEvents {
    ...
    // YYY Send a application data JSON message? This is not "Signaling".
    public void send(String msg) {
        handler.post(new Runnable() {
            @Override
            public void run() {
                JSONObject json = new JSONObject();
                jsonPut(json, "type", "app-event");
                jsonPut(json, "action", "ping");
                wsClient.send(json.toString());
                JSONObject json2 = new JSONObject();
                jsonPut(json2, "type", "app-event");
                jsonPut(json2, "action", "feedback");
                jsonPut(json2, "bandwidth", 1234);
            }
        });
    }
    ...
    public void onWebSocketMessage(final String msg) {
        ...
        try {
            ...
            if (msgText.length() > 0) {
                ...
                } else if (type.equals("app-event")) { // TODO: onAppWebSocketMessage (as opposed to Signaling messages)
                    String action = json.optString("action");
                    Log.d(TAG, "WebSocket action, App-Event!");
                    if (action.equals("ping")) {
                        if (!stopwatch.isRunning()) {
                            stopwatch.reset();
                            stopwatch.start();
                            Log.d(TAG, "RTT elapsed: " + stopwatch.elapsed(TimeUnit.MILLISECONDS));
                            send("pong");
                        }
                    }
                    else if (action.equals("pong")){
                        Log.d(TAG, "RTT: " + stopwatch.elapsed(TimeUnit.MILLISECONDS));
                        stopwatch.stop();
                    }
                }
                else {
